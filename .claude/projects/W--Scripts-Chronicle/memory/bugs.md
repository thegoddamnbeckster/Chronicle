...
**Deploy trap (own mistake, worth recording):** Two Hardcover plugin directories exist on disk — `plugins/hardcover/` and `plugins/chronicle.plugin.hardcover/` — the same short-id-vs-canonical-id split already seen once this session with TMDB (BUG-036/duplicate-plugin-directory notes). The DB's `plugins.DllPath` for hardcover points at `src/Chronicle.API/plugins/hardcover/Chronicle.Plugin.Hardcover.dll` (confirmed via `PluginHostService`/`PluginService`, which load from `plugin.DllPath` verbatim, no hash check). Redeployed the fix there twice to a *different* (repo-root `plugins/hardcover/`) directory that Chronicle never actually loads from — both the completeness-tiebreaker fix and the slug-lookup fix silently never took effect until this was caught. `plugins/chronicle.plugin.hardcover/` also exists with a stale June-12-era DLL and is seemingly unreferenced by the DB row — likely safe to delete like the TMDB duplicate was, not yet done.
